...
TABLE2 = {
    "id_00": {"ddae": 231, "ad_train": 643, "ad_test": 275},
    "id_02": {"ddae": 225, "ad_train": 624, "ad_test": 267},
    "id_04": {"ddae": 162, "ad_train": 448, "ad_test": 192},
    "id_06": {"ddae": 229, "ad_train": 637, "ad_test": 272},
}
...
def extract_features(wav_path):
    ...
    basic = np.array(
        [rms, kurtosis(y), skew(y), ptp, crest, np.mean(y), np.std(y), ent],
        dtype=np.float32,
    )
...
def load_or_extract(snr, id_name, kind):
    ...
    if len(wavs) == 0:
        raise RuntimeError(f"No wavs found: snr={snr}, id={id_name}, kind={kind}\nROOT={ROOT}")
...
def compute_test_counts(cfg, Xn_len, Xa_len):
    desired_ab = cfg["ad_test"] // 2
    desired_no = cfg["ad_test"] - desired_ab

    max_no = max(0, Xn_len - cfg["ad_train"])
    max_ab = max(0, Xa_len)

    ab = min(desired_ab, max_ab)
    no = min(desired_no, max_no)
...
def main():
    ...
    print(f"Noise factors = {NFS}")

    print("\n[DDAE TRAIN DATA]")
    total = 0
    for i in IDS:
        print(f"{i} : {TABLE2[i]['ddae']}")
        total += TABLE2[i]["ddae"]
    print("-"*25)
    print(f"TOTAL : {total}")
